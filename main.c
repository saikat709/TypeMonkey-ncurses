...
void animateForwardFull(int max_y,  int max_x);
void animateBackwardHalf(int max_y, int max_x);
void animateForwardHalf(int max_y,  int max_x);
void animateForward(int max_y,       int max_x);
void animateBackward(int max_y,      int max_x);
void showGameTitleSmall();
void printTypeMonkey();
// void printInMiddle();
void printInMiddleAnimate(WINDOW *win, int starty, int startx, int width, char *string, chtype color);
void animatePrintTitle(int starty, int startx, const char *title);
...
int main(){
    ...
    animateForwardFull(max_y, max_x);
    printTypeMonkey();
...
void animateBorders(WINDOW *win, int delay_ms) {
    int height, width;
...
void getRandomWords(int level){
    ...
    for (int i = 0; i < words_size; ++i) {
        ...
        strcpy(selectedWords[i], WORDS[level][random_index]);
    }
}
...
void homeScreen(){
    ...
    animateBackwardHalf(max_y, max_x);
    animateForwardHalf(max_y, max_x);
    ...
    animatePrintTitle(LINES/3 - 5, COLS*3/4 - length/2, title);
    animatePrintTitle(LINES/3 - 4, COLS*3/4 - length/2, titleUnderline);
    ...
    for(i = 0; i < n_choices; ++i) {
        ...
        printInMiddleAnimate(my_menu_win, 2 + i*4, 1, 40, choices[i], 
    ...
    while((c = wgetch(my_menu_win)) != KEY_F(1) && isScreen)  // F1 to exit
    {
        ...
        for(i = 0; i < n_choices; ++i) {
            ...
            printInMiddle(my_menu_win, 2 + i*4, 1, 40, choices[i], 
                        selectedIndex == i ? COLOR_TEXT_MENU_SUCCESS | A_BOLD : COLOR_TEXT_WHITE );
...
void typingScreen(int level) {
    ...
    while(isScreen && (c = getch()) != KEY_F(1)) {
        ...
        for( int i = 0; i < numWords; ++i ){
            ...
            for( int j = 0; j < len; ++j ){
                if ( i < currentWordIndex || (i == currentWordIndex && j < currentLetterIndex) ){
                    if ( typedWords[i][j] == selectedWords[i][j] ){
                ...
                mvprintw(row, COLS/3 + 20 + column, "%c", selectedWords[i][j] );
        ...
        ACCURACY =  correctCharsTyped*100.0/(correctCharsTyped + inCorrectCharsTyped);
        ...
        mvprintw(row + 4,  COLS/3 + 20, "WORD: " );
        attron(A_UNDERLINE | A_BLINK  );
        mvprintw(row + 4,  COLS/3 + 30, "%s        ", typedWords[currentWordIndex] );
        ...
        for( int i = 0; i <= numWords; ++i){
            ...
            for( int j = 0; j < len; ++j ){
                if ( i < currentWordIndex ){
                    ...
                    if (j < currentLetterIndex ) mvprintw( row, COLS/3 + 20 + column, "%c", typedWords[i][j] );
        ...
        if (c != ERR) {
            ...
            if (c == 27) {  // EScape
                ...
                if ( currentLetterIndex == 1 && currentWordIndex == 0 ){
                    typedWords[currentWordIndex][--currentLetterIndex]  = '\0';
                }
                    ...
                    currentWordIndex--;
                    currentWordIndex = strlen(typedWords[currentWordIndex]);
                ...
                if ( currentWordIndex == numWords && currentLetterIndex == strlen(selectedWords[numWords-1]) ){
                    typingEndScreen();
                    isScreen = false;
                    break;
                }
            }
            else if ( c != ' ' && typedWords[currentWordIndex][currentLetterIndex-1] == ' ' ) {
                if ( selectedWords[currentWordIndex][currentLetterIndex] != c ) inCorrectCharsTyped++;
                currentWordIndex++;
                currentLetterIndex = 0;
                typedWords[currentWordIndex][currentLetterIndex++] = c;
                typedWords[currentWordIndex][currentLetterIndex] = '\0';
            }
            else {
                typedWords[currentWordIndex][currentLetterIndex++] = c;
